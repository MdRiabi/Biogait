...
class ManagementPage:
    ...
    def enrollment_dialog(self):
        ...
        with ui.dialog() as dialog, cyber_card().classes('w-96'):
            ...
            ui.label('Vidéos de démarche (min 5)').classes('text-xs text-muted mt-2')
            ui.upload(label='Uploader Vidéos .mp4', on_upload=lambda e: ui.notify(f'Vidéo {e.name} reçue'), multiple=True).classes('w-full mt-2')
    ...
    def process_enroll(self, name, dialog):
        ui.notify(f"Démarrage de l'analyse IA pour {name}...", type='info')
        # Ici on appellerait le pipeline backend
        dialog.close()
